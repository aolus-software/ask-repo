...
def test_line_numbers_correct_with_production_overlap() -> None:
    """Production constants (chunk_size=1200, chunk_overlap=150) with a single long
    function that forces the splitter to cut mid-body, engaging overlap. This test
    catches the cursor-advance bug: with overlap, the next piece starts before the
    prior piece ends, so cursor must advance minimally to find it."""
    # A single function with ~120 similar long lines, over 1200 chars, forcing overlap
    body = "\n".join(
        f"    value_{i} = compute_something_with_a_longish_name({i}) + offset_{i}"
        for i in range(120)
    )
    large_source = "def big() -> int:\n" + body + "\n    return 0\n"
    ...
    # Critical: every chunk's text must be contained in the window defined by its
    # start_line and end_line. This is the gate for the cursor-advance fix.
    for chunk in chunks:
        assert 1 <= chunk.start_line <= chunk.end_line <= len(lines), (
        ...
        # The chunk's text must appear in the window (the core assertion for citations)
        assert chunk.text.strip() in window, (
            f"Chunk {chunk.chunk_index} text not found in window at "
            f"lines {chunk.start_line}-{chunk.end_line}. "
            f"Chunk text: {chunk.text[:60]!r}"
        )

...
async def test_soft_delete_for_project_cascades(db_session: AsyncSession) -> None:
    """Soft-deleting a project cascades to all its modules' records."""
    ...
    assert count == 2
    assert await repo.list_for_module(module_a.id) == []
    assert await repo.list_for_module(module_b.id) == []

...
- **Does the AI call log (§2.1) need per-call cost, or only per-call tokens?** Tokens are reported by the provider and are a fact; a cost figure needs a price table per provider and per model, which goes stale silently the moment a vendor changes it — and a wrong number shown next to a real invoice is worse than no number. Storing tokens and pricing them at read time keeps the stale part in one place, but it means the log alone cannot answer "what did last month cost" without that table. Related: whether an instance running a local model logs at all, where the token counts are real but the spend is always zero.

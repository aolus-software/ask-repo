...
export function LastOwnerNotice({ error }: { error: unknown }) {
  ...
  const projects = (error.detail?.projects ?? []) as BlockingProject[];
...
> Check `ApiError`'s constructor in `frontend/lib/api/errors.ts` — `form-dialog.test.tsx` passes a fourth argument as a `fields` map. If the extra payload is exposed under a different property than `detail`, adjust `LastOwnerNotice` to read the one that exists rather than adding a new one.

...
`NOT_PROJECT_OWNER` **stays in `ErrorCode`** — values are a wire contract and members are never removed. If nothing raises it any more, leave the member with a comment saying so.

...
- Produces: `Role`, `RolePermission`, `ProjectMembership` ORM classes.
...
- [ ] **Step 8: Run the full suite and record the fallout**

Run: `cd backend && uv run pytest 2>&1 | tail -40`

Expected: **many failures**, all of the same shape — tests that created a project as user A and asserted user B could read it. That is the behaviour change. Do not weaken an assertion to make one pass. For each failing test, either:
- it asserts phase-1 sharing → rewrite it to assert the new behaviour (non-member gets `404`), or
- it needs access to do its real job → add `await grant_membership(user.id, project.id)`.

`test_scoping_is_single_point.py::test_no_module_compares_created_by_to_an_actor` is also expected to fail here — Task 6 fixes it.
...
git add backend/app/core/access.py backend/app/core/errors.py \
        ...
        backend/tests/test_scoping_is_single_point.py
...
Inject `RoleRepository` as `self._roles` in `ProjectService.__init__`, following how `self._repository` is already built.
...
async def test_a_deactivated_users_memberships_are_left_intact(
    ...
    members = (await client_for_user_a.get(f"/projects/{project_id}/members")).json()
    assert str(authed_user.id) in [entry["userId"] for entry in members]
```
...
```python
    async def ownerless_project_ids(self) -> Sequence[uuid.UUID]:
        ...
        result = await self.session.execute(
            ...
            .outerjoin(
                Role, (Role.id == ProjectMembership.role_id) & (Role.name == "owner")
            )
            .outerjoin(
                User,
...
async def _restore_system_roles() -> None:
    async with get_sessionmaker()() as session:
        ...
        for name, permissions in SYSTEM_ROLES.items():
            ...
            await repository.replace_permissions(role.id, [p.value for p in permissions])
        await session.commit()
